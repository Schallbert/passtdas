...
def handle_arc_commands(previous_coordinates, line):
    ...
    if line.find('G02') > -1:
        print(coordinates)
        previous_coordinates.append(coordinates[3])
        previous_coordinates.append(coordinates[4])
    ...
    radius = sqrt((coordinates[0] - coordinates[3]) ** 2 + (coordinates[1] - coordinates[4]) ** 2)
    ...
    with file as f:
        ...
        for line in lines:
            ...
            if has_arc_command(line):
                lines = handle_arc_commands(previous_line, line)
                if lines:
                    data.extend(lines)
            if data:
                previous_line = data[-1]
